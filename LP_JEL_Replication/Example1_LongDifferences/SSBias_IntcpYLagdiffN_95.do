...
qui forvalues i = 1(1)$nreps{
	...
	*** Estimate the AR model ***> without knowing the shock
	reg y l.y /*l.d.y*/  // cons and N lag diff
	scalar rhoe = _b[L.y]
	scalar ae = 0
	//scalar ae = _b[L.D.y]
...
*		local b_ldiff_`h' = _b[D.y] 
		reg f`h'y vy /*l.d.y*/  // cons and N lag diff
...
*		reg f`h'y d.y 
*		local b_ldiff_`h' = _b[D.y] 
		reg df`h'y vy  /*l.d.y*/  // cons and N lag diff
...
gr export "SSBias_IntcpYLagdiffN_rho_`C'_reps_`A'_obs_`B'.pdf", replace
