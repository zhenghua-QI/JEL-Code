...
forv h=0/4 {
	...
	*** LPIV
		xi: xtivreg2 D`h'y (dCAPB = size) _x* if boom==1 , fe cluster(iso)
		estimates store LP`h'
		eststo LP`h'
		
	replace h = _n-1						if _n-1==`h'
	replace Zero = 0 						if h==`h' | l.h==`h'
	replace b = _b[dCAPB] 					if h==`h'
	replace d = b+1.96*_se[dCAPB]	if h==`h'
	replace u = b-1.96*_se[dCAPB]	if h==`h'
}
...
gen FE = I*1000+H // e.g. USA at horizon 1 is 111001 etc. 
	*** LPIV stacked
	tsset FE year
	noi xi: ivreg2 Y (T_* = Z_*) X* i.FE if boom==1 , cluster(FE) // dkraay(3) //*
...
ytitle("Response, log Real GDP per capita x100, {fontface cmsy10: R}({it:h})", size(medsmall)) xtitle("Horizon, years, {it:h} ", size(medsmall)) ///
...
text(1.9 2 "Joint test, {fontface cmsy10: R}({it:h})=0:" " " "{it:{&chi}}{superscript:2}(`df_joint')=`:display %5.1fc `chi2_joint'' ({it:p}=`:display %5.3fc `p_joint'')") ///
xsize(2) ysize(2.3) xsc(r(0 5.4)) xlab(0 1 2 3 4 5 "average") ysc(r(-5 2)) ylab(-5 -4 -3 -2 -1 0 1) scale(1.2) ///
graphregion(color(white)) plotregion(color(white)) ///
name(Rboom,replace)
gr export Rboom.pdf , replace
