...
forv h=0/4 {
	...
	*** LPIV
		xi: xtivreg2 D`h'y (dCAPB = size) _x* if boom==0 , fe cluster(iso)
...
gen FE = I*1000+H // e.g. USA at horizon 1 is 111001 etc. 
	*** LPIV stacked
	tsset FE year
	noi xi: ivreg2 Y (T_* = Z_*) X* i.FE if boom==0 , cluster(FE) // dkraay(3) //*
...
ytitle("Response, log Real GDP per capita x100, {fontface cmsy10: R}({it:h})") ///
xtitle("Horizon, years, {it:h} ") ///
